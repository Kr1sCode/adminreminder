...
jednym, szybkim i ładnym miejscu — zamiast listy w SharePoint / Excelu:
...
- **Punkty TLS** — LDAPS, VPN, Exchange, RD Gateway i inne usługi na dowolnym
  porcie (data ważności + walidacja łańcucha, EKU, nazwy, opcjonalny pin SHA-256),
- **Gwarancje sprzętu**,
...
- i dowolne inne pozycje z datą ważności.

![Dashboard](screenshots/AR_dashboard.jpg)

## ✨ Główne cechy

- 🔐 Logowanie + role (**admin** / **viewer**), **MFA (TOTP)**
- 🏷️ Wiele typów pozycji: Certyfikat HTTPS, Punkt TLS (LDAPS/usługa), Gwarancja,
...
- 🗂️ Inwentarz kont z **Active Directory** (LDAP) i **Entra ID** (Graph)
...
Debian 13, instaluje natywnie Node.js + aplikację, zakłada **pustą bazę** i usługę
...
Po zakończeniu instalator wypisze adres `http://<IP-kontenera>:3000`. Przy pierwszym
otwarciu aplikacja poprosi o utworzenie konta administratora.

Aktualizacja: `pct exec <CTID> -- update`

> Pliki w formacie repozytorium community-scripts (pod przyszły PR) znajdziesz w
